...
parser.add_argument('--load_model_path', default=f'./checkpoints/bs64lr001loss4/latest.ckpt', help='加载模型的路径, 要精确到ckpt文件')
parser.add_argument('--save_iter', default=500, help='在训练时每多少iter打印一次损失并保存checkpoint; 在测试时每多少iter打印一次累计至当前iter的平均acc和recall')
parser.add_argument('--num_workers', default=0, help='同时启用多个进程在cpu加载数据, 以平衡加载数据和训练数据的速度差距, 推荐为cpu物理核心数-1或-2')
parser.add_argument('--prefetch_factor', default=2, help='每个进程预加载的batch数量, 配合num_workers尽可能保证gpu不空闲')
